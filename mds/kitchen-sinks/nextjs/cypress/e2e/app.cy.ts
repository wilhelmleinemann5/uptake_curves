describe('nextjs', () => {
  ...
  it('renders mc-components', () => {
    cy.get('mc-top-bar').find('.product').should('contain.text', 'Next.js');
    cy.get('mc-side-bar').should('contain.text', 'Home');
    ...
    cy.get('mc-calendar').find('[data-cy="month-name"]').click();
    cy.get('mc-input-date').find('[data-cy="input"]');
    cy.get('mc-date-range').as('mc-date-range');
    cy.get('@mc-date-range').find('[data-cy="from-input"]').find('[data-cy="input"]').click({ force: true });
    cy.get('@mc-date-range').find('[data-cy="to-input"]').find('[data-cy="input"]').click({ force: true });
