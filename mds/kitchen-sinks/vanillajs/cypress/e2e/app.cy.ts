describe('vanillajs', () => {
  ...
  it('renders mc-components', () => {
    cy.get('mc-top-bar').find('.product').should('contain.text', 'VanillaJS');
    cy.get('mc-side-bar').should('contain.text', 'Form example');
    ...
    cy.get('mc-pagination').find('ul').children().should('have.length.gt', 5);
    ...
    cy.get('mc-drawer').find('[dialogaction=cancel]').click();
