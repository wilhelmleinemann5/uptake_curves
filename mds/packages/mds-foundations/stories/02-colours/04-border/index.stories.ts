...
export const Border = (args: Args, context: StoryContext) => {
  return html`${unsafeHTML(generateThemeSelector())}
    <style>
      .css-example > div {
        border-width: 2px;
        border-style: var(--mds_global_border_style);
      }
    </style>
    ${renderComponentBanner(
      'Border colour classes',
      html`<p>
        These helper classes allow you to quickly apply a theme's <strong>border</strong> colours to an element and
        align with a
