...
export const CustomColumns = (args: Args, context: StoryContext) => {
  return html`${unsafeHTML(hologramStyles(args))}
    <div class="wrapper">
      ${unsafeHTML(
        cssClassNamesGeneratorBreakpoint('mds-grid-%breakpoint%-cols-%i%', 'grid-template-columns: repeat(%i%, auto);'),
      )}
      ${unsafeHTML(templateStory(args, example, 'mds-grid-cols-4'))} ${renderCodePreview(preview, context)}
    </div>`;
};
