...
export const ColumnPositioningReverse = (args: Args, context: StoryContext) => {
  return html`${unsafeHTML(hologramStyles(args))}
    <div class="wrapper">
      ${unsafeHTML(
        cssClassNamesGeneratorBreakpoint(`mds-grid-%breakpoint%-col-%i%-reverse`, 'grid-column-start: -%i+1%;'),
      )}
