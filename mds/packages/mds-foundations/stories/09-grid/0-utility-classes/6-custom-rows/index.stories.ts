...
export const CustomRows = (args: Args, context: StoryContext) => {
  return html`${unsafeHTML(hologramStyles(args))}
    <div class="wrapper">
      ${unsafeHTML(
        cssClassNamesGeneratorBreakpoint(
          'mds-grid-%breakpoint%-rows-%i%',
          'grid-template-rows: repeat(%i%, minmax(0, auto);',
        ),
      )}
      ${unsafeHTML(templateStory(args, example, 'mds-grid-rows-4'))} ${renderCodePreview(preview, context)}
    </div>`;
};
