...
export const CustomColumnGaps = (args: Args, context: StoryContext) => {
  return html`${unsafeHTML(hologramStyles(args))}
    <div class="wrapper">
      ${unsafeHTML(cssClassNamesGeneratorDensityBreakpoint('mds-%breakpoint%-col-gap-%i%', 'column-gap: %i%;'))}
      <div class="main-container">${unsafeHTML(templateStory(args, example, gridCssClasses))}</div>
