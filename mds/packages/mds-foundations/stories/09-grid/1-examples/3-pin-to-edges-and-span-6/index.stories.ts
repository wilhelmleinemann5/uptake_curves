...
import { preview, example, gridCssClass } from './code-preview';
...
export const PinToEdgesAndSpanOverHalf = (args: Args, context: StoryContext) => {
  return html`${unsafeHTML(hologramStyles(args))}
    <div class="wrapper">
      ${renderComponentBanner('MDS grid system', componentBanner)}
      ${unsafeHTML(templateStory(args, example, gridCssClass))} ${renderCodePreview(preview, context)}
    </div>`;
};
