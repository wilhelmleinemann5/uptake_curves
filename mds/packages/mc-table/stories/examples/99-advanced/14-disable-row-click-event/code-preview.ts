...
function saveToFavourites(row) {
  alert('Row saved to favourites: ' + JSON.stringify(row));
}

// Add buttons to favourite column
mcTable.innerHTML = data.map(row => 
  \`<mc-button icon="star" hiddenlabel slot="\${row.id}_favourite" onclick="saveToFavourites(row)})">
    Edit
  </mc-button>\`
).join('');
...
const columns: Array<TableColumn> = [
  ...
  },
  {
    ...
    width: '50px',
    sortDisabled: true,
    rowClickDisabled: true, // This prevents row click event from firing when clicking in this column
  }
];
