export const apiServiceCode = `export interface City {
  postalcode: string;
  name: string;
  countryCode: string;
  geonameId: number;
  ...
  public async search(value: string): Promise<void> {
    if (value != this.searchQuery || this.startRow > 0) {
      ...
      const result = await fetch(
        \`https://secure.geonames.org/searchJSON?name_startsWith=\${this.searchQuery}&featureCode=PPL&maxRows=\${this.maxRows}&startRow=\${this.startRow}\`,
        {
          method: 'GET',
          mode: 'cors',
          headers: {
